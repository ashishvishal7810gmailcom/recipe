...
function RenderCourse({course, description, data, currentContentAsHTML}) {
        return(
            <React.Fragment>
                ...
                <div className="col-12 col-md-6">
                    ...
                    //     convertFromRaw(JSON.parse(course.description)).getCurrentContent().getPlainText()
                    // );
...
class CourseDetails extends Component {
    ...
    componentDidMount() {
        this.props.fetchCourse('purchased',this.props.courseId);
    }
    ...
    render() {
        ...
        else if (this.props.course.course.length != 0)   
        {
            ...
            const topics = this.props.course.course.topics.map((topic) => {
                return (
                    <li>
                        <Link to={`/purchased/${this.props.courseId}/${topic._id}`} className="text-decoration-none">
                            {topic.title}
                        </Link>
                    </li>
                )
